...
import Data.List (sort, elemIndices, foldl')
...
findSpacesHelper :: [Int] -> [Int] -> [Int]
findSpacesHelper x1 x2 = elemIndices True xs
  where xs = map (\x -> (64 < x) && (x < 123)) $ zipWith (\x y -> x `xor` y) x1 x2

findSpaces :: [Int] -> [[Int]] -> [Int]
findSpaces ct cts = outs
  where n = length cts
        inds = concatMap (findSpacesHelper ct) cts
        freq = MS.toOccurList $ MS.fromList $ inds
        outs = map (fst) $ filter (\x -> snd x > (n `div` 2)) freq

setKeyHelper :: [Int] -> [Int] -> [Int] -> [Int]
...
main = do
  ...
  let key = replicate 350 0
  let cs = map parseFromHex cipherTexts

  let key' = foldl' (setKey cs) key (take 10 cs)
  let fixedKey = foldl' replaceNth key' fixes

  let rr = map (decipher fixedKey) cipherTexts
  print $ last rr

  -- calculation for the exam
  let exam = "6c73d5240a948c86981bc294814d"
  let plain = map ord "attack at dawn"
  let key = zipWith (\x y -> x `xor` y) plain $ parseFromHex exam
